      ...
      >
        ...
        >
          ...
          >
            ...
            {letter.type === "stranger" &&
              (letter.isClaimed || letter.isRead) && (
                <span
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
        ...
        >
          ...
          {isSentLetter &&
            (() => {
              ...
              const showBreakdown =
                uniqueOpens > 0 && (emailOpens > 0 || platformOpens > 0);
              return (
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <span
                    style={{
                      ...
                      border: `1px solid ${
                        hasOpens
                          ? "rgba(90,158,122,0.25)"
                          : "rgba(28,26,23,0.1)"
                      }`,
                    }}
                  >
                    {hasOpens
                      ? `👁 Opened by ${uniqueOpens} ${
                          uniqueOpens === 1 ? "person" : "people"
                        }`
                      : "· Not opened"}
                  </span>
                  {showBreakdown && (
            ...
            <button
              ...
              style={{
                ...
                border: editHov
                  ...
                  : "1.5px solid #E0D4BC",
                color: editHov ? "var(--gold)" : "var(--ink-soft)",
                background: editHov ? "#fef9f2" : "transparent",
              }}
            >
              ✏ Edit
            </button>
            <button
              ...
              style={{
                ...
                border: delHov
                  ? "1.5px solid var(--tc)"
                  : "1.5px solid #f5d4ce",
                color: delHov ? "#fff" : "var(--tc)",
                background: delHov ? "var(--tc)" : "transparent",
              }}
            >
              ✕ Delete
            </button>
...
function EditModal({ letter, onSave, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[560px] mx-4 overflow-hidden animate-fade-up"
        style={{
      ...
      >
        <div
          ...
          style={{ borderBottom: `1px solid ${FT}` }}
        >
          <span
            style={{
              fontFamily: '"Lora", serif',
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center cursor-pointer bg-transparent border-none text-[20px]"
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          {error && (
            <div
              className="text-[12px] px-3 py-2 rounded-[7px]"
              style={{
                color: "var(--tc)",
          ...
          <div className="flex gap-2 justify-end">
            <button
              onClick={onClose}
              className="px-5 py-[9px] rounded-pill font-sans text-[13px] cursor-pointer bg-transparent"
              style={{ color: "var(--ink-muted)", border: `1px solid ${BD}` }}
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-5 py-[9px] rounded-pill font-sans text-[13px] font-medium border-none cursor-pointer disabled:opacity-50"
...
function DeleteModal({ letter, onConfirm, onClose, deleting }) {
  return (
    ...
    >
      <div
        className="w-full max-w-[400px] mx-4 overflow-hidden animate-fade-up"
        style={{
          background: "var(--cream)",
          border: `1px solid ${BD}`,
          borderRadius: 16,
      ...
      {cta && (
        <button
          ...
          style={{
            ...
            fontWeight: 500,
            cursor: "pointer",
            boxShadow: "0 4px 14px rgba(139,58,42,0.25)",
          }}
        >
          {cta}
        </button>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 8,
        border: `1px solid ${
          active ? "rgba(196,99,58,0.35)" : "rgba(28,26,23,0.1)"
        }`,
      ...
      {count != null && (
        <span
          style={{
            ...
            borderRadius: 20,
            fontWeight: 600,
            background: active ? "rgba(196,99,58,0.12)" : "rgba(26,18,8,0.05)",
...
function ReceivedLetterCard({ letter, onOpen }) {
  ...
  const displaySender =
    senderName && senderName !== "—" ? senderName : senderEmail;
    ...
    >
      ...
      <div
        style={{
          ...
          bottom: 0,
          width: 4,
          background: "linear-gradient(180deg, var(--purple), var(--tc))",
          borderRadius: "4px 0 0 4px",
        }}
      />
...
const CARD_CONFIG = {
  ...
  stranger: {
    ...
    tagLabel: "Caring Stranger",
    tagBg: "rgba(122,158,142,0.1)",
    tagColor: "var(--sage)",
    tagBorder: "rgba(122,158,142,0.25)",
  },
  read: {
    accentGrad: "linear-gradient(180deg, var(--purple), var(--gold))",
    tagLabel: "Listener Read",
    tagBg: "rgba(139,126,200,0.1)",
    tagColor: "var(--purple)",
    tagBorder: "rgba(139,126,200,0.25)",
  },
  sent: {
    accentGrad: "linear-gradient(180deg, var(--gold), var(--ink-muted))",
    tagLabel: "Send to Someone I Know",
    tagBg: "#EDE5D4",
...
export default function MySpacePage() {
  ...
  useEffect(() => {
    setPersonal(personalLetters);
  }, [personalLetters]);
  useEffect(() => {
    setStranger(ownStrangerLetters);
  }, [ownStrangerLetters]);
  ...
  const TABS = [
    ...
    { id: "received", label: "For Me", count: receivedLetters.length },
    ...(canWriteStranger
      ? [
          {
            id: "stranger",
            label: "Send to a Stranger",
            count: stranger.length,
          },
        ]
      : []),
    ...(canReadFeed
      ? [{ id: "read", label: "Held by Me", count: readLetters.length }]
      : []),
  ];
  ...
  const visibleLetters = (() => {
    if (activeTab === "personal")
      return personal.map((l) => ({ ...l, _cardType: "personal" }));
    if (activeTab === "sent")
      return sentLetters.map((l) => ({ ...l, _cardType: "sent" }));
    if (activeTab === "received")
      return receivedLetters.map((l) => ({ ...l, _cardType: "received" }));
    if (activeTab === "stranger")
      return stranger.map((l) => ({ ...l, _cardType: "stranger" }));
    if (activeTab === "read")
      return readLetters.map((l) => ({ ...l, _cardType: "read" }));
    // all tab
    return [
  ...
  async function handleDeleteConfirm() {
    setDeleting(true);
    try {
      const res = await apiFetch(`/api/letters/${deletingLetter._id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  return (
    <main
      className="page-enter w-full flex justify-center px-4 sm:px-6"
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span
              style={{
                ...
                letterSpacing: "0.6px",
                textTransform: "uppercase",
                background: `${ROLE_COLOR[userRole]}12`,
                color: ROLE_COLOR[userRole],
                border: `1px solid ${ROLE_COLOR[userRole]}35`,
              }}
            >
              <span
                style={{
                  ...
                  height: 5,
                  borderRadius: "50%",
                  background: ROLE_COLOR[userRole],
                }}
              />
              {ROLE_LABEL[userRole]}
            </span>
        ...
        {visibleLetters.length > 0 && (
          ...
          >
            ...
            >
              ...
              {visibleLetters.length === 1 ? "letter" : "letters"}
            </span>
            <span
              style={{
                fontSize: 12.5,
        ...
        {visibleLetters.length === 0 ? (
          activeTab === "personal" ? (
            <EmptyState
              ...
              subtitle="Write to yourself — your past self, your future self. Even one sentence matters."
            ...
            <EmptyState
              icon="📬"
              title="No letters sent yet"
              subtitle="Write a letter to someone you know and send it directly to their inbox."
              cta="Write a letter"
              onCta={() => navigate("write")}
            />
          ) : activeTab === "received" ? (
            <EmptyState
              icon="📥"
              title="No letters received yet"
              subtitle="When someone sends you a letter through Letter from Heart, it will appear here."
            />
          ) : activeTab === "stranger" ? (
            <EmptyState
              icon="🌍"
              title="No stranger letters yet"
              subtitle="Share something with the world. Your words might be exactly what someone needs today."
              cta="Write to a stranger"
              onCta={() => navigate("write")}
            />
          ) : activeTab === "read" ? (
            <EmptyState
              icon="🎧"
              title="No letters read yet"
              subtitle="Someone out there is waiting to be heard. Go to the listener feed and be present."
              cta="Go to Listener Read"
              onCta={() => navigate("listenerread")}
            />
          ) : (
            <EmptyState
              icon="✦"
              title="Your space is empty"
              subtitle="Start by writing a letter — to yourself, to a stranger, or to someone you love."
              cta="Write your first letter"
              onCta={() => navigate("write")}
            />
          )
            ...
            {visibleLetters.map((letter) => {
              ...
              return (
                <LetterCard
                  ...
                  onOpen={openLetterPanel}
                  onEdit={
                    letter.type === "personal" ? setEditingLetter : undefined
                  }
                  onDelete={
                    letter.type === "personal" ? setDeletingLetter : undefined
                  }
                />
              );
            })}
...
function ComposeTeaserBar({ onClick }) {
  ...
  return (
    <div
      ...
      style={{
        marginTop: 28,
        border: `1.5px dashed ${hov ? "var(--tc)" : BD}`,
        borderRadius: 14,
        padding: "22px 28px",
        display: "flex",
        alignItems: "center",
        gap: 16,
        cursor: "pointer",
        background: hov ? "rgba(139,58,42,0.03)" : "transparent",
    ...
    >
      ...
      <div>
        ...
        >
          No pressure. Start with "Dear…" and see where it goes.
        </div>
      </div>
    </div>
  );
