...
const MOODS = [
  ...
  { id: 'gratitude', emoji: '🌿', label: 'Gratitude',        activeStyle: { background: '#5A5580', borderColor: '#5A5580' } },
    ...
    <div className="mb-6">
      <div className="flex items-center justify-between mb-2">
        ...
        {onBack && (
          <button
            onClick={onBack}
            style={{ fontSize: 12.5, fontFamily: '"Inter", system-ui, sans-serif', color: 'var(--ink)', fontWeight: 500, background: 'rgba(59,54,99,0.06)', border: '1px solid rgba(59,54,99,0.12)', borderRadius: 7, cursor: 'pointer', padding: '5px 12px', display: 'flex', alignItems: 'center', gap: 5, transition: 'background 0.15s' }}
      ...
      <h2 style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 'clamp(20px, 2.5vw, 26px)', fontWeight: 700, color: 'var(--ink)', lineHeight: 1.2, letterSpacing: '-0.3px', marginBottom: 6 }}>
        {title}
      </h2>
      <p style={{ fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', fontSize: 13.5, color: 'var(--ink-muted)', lineHeight: 1.6 }}>
        {subtitle}
      </p>
    </div>
  )
            ...
            <div
              ...
              style={{
                background: disabled
                  ? 'rgba(59,54,99,0.02)'
                  : active
                  ? m.id === 'stranger' ? 'rgba(46,125,91,0.06)' : m.id === 'known' ? 'rgba(244,129,63,0.06)' : 'rgba(92,79,168,0.07)'
                  : 'var(--paper)',
                border: active
                  ? m.id === 'stranger' ? '1.5px solid var(--sage)' : m.id === 'known' ? '1.5px solid var(--tc)' : '1.5px solid var(--purple)'
                  : '1px solid rgba(59,54,99,0.09)',
                borderRadius: 12,
                padding: '18px 18px',
                cursor: disabled ? 'not-allowed' : 'pointer',
                opacity: disabled ? 0.42 : 1,
                transform: active ? 'translateY(-2px)' : '',
                boxShadow: active ? '0 6px 20px rgba(59,54,99,0.08)' : 'none',
                transition: 'all 0.2s',
              }}
            >
              <span style={{ fontSize: 24, display: 'block', marginBottom: 10 }}>{m.icon}</span>
              <div style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 15, fontWeight: 700, color: 'var(--ink)', marginBottom: 5, lineHeight: 1.2 }}>{m.name}</div>
              <div style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.65, marginBottom: 10 }}>{m.desc}</div>
              <div
                style={{
                  fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', fontSize: 11,
                  color: disabled ? 'var(--sage)' : 'var(--ink-muted)',
                  paddingTop: 10, borderTop: '1px solid rgba(59,54,99,0.07)',
                  lineHeight: 1.5,
                }}
              >
      ...
      >
        ...
        <input
          ...
          style={{
            width: '100%',
            padding: '10px 14px',
            fontFamily: '"Inter", system-ui, sans-serif', fontSize: 13.5, fontStyle: 'italic', color: 'var(--ink)',
            background: 'var(--paper)', borderRadius: 8, outline: 'none',
            border: '1px solid rgba(59,54,99,0.15)', transition: 'border-color 0.2s',
          }}
          placeholder="their@email.com"
          value={email}
          onChange={e => setEmail(e.target.value)}
          onFocus={e => (e.target.style.borderColor = 'var(--tc)')}
          onBlur={e => (e.target.style.borderColor = 'rgba(59,54,99,0.15)')}
        />
      </div>
        ...
        {MOODS.map(m => (
          <div
            ...
            style={{
              ...
              transition: 'all 0.2s',
              ...(mood?.id === m.id
                ? { ...m.activeStyle, color: 'white', border: `1px solid ${m.activeStyle.background || 'transparent'}`, transform: 'translateY(-1px)', boxShadow: '0 6px 18px rgba(59,54,99,0.12)' }
                : { background: 'var(--paper)', border: '1px solid rgba(59,54,99,0.09)', color: 'var(--ink-soft)' }
              ),
            }}
          >
    ...
    <div className="animate-fade-up">
      ...
      {mood && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          <span style={{ fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', fontSize: 13, color: 'var(--ink-muted)' }}>
            This letter carries:
          </span>
          <span style={{
            ...
            color: 'white',
            fontSize: 12, fontFamily: '"Inter", system-ui, sans-serif', fontWeight: 500,
          }}>
      ...
      <div
        ...
        style={{ boxShadow: '0 2px 8px rgba(59,54,99,0.04), 0 12px 40px rgba(59,54,99,0.08), 0 0 0 1px rgba(59,54,99,0.06)' }}
      >
        ...
        <div className="relative z-[2]" style={{ padding: 'clamp(28px, 5vw, 56px) clamp(20px, 4vw, 56px) clamp(28px, 5vw, 56px) clamp(28px, 5vw, 80px)' }}>
          ...
          <div className="flex items-baseline mb-7" style={{ gap: 6 }}>
            <span style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 22, fontWeight: 500, color: 'var(--ink)', flexShrink: 0 }}>Dear </span>
            <input
              style={{
                fontFamily: '"Inter", system-ui, sans-serif', fontSize: 22, fontWeight: 500,
                color: 'var(--tc)', background: 'transparent',
                border: 'none', borderBottom: '1.5px solid rgba(244,129,63,0.2)',
              ...
              onBlur={e => (e.target.style.borderBottomColor = 'rgba(244,129,63,0.2)')}
            />
            <span style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 22, fontWeight: 500, color: 'var(--ink)', flexShrink: 0 }}>,</span>
          </div>
          ...
          <textarea
            style={{
              ...
              fontFamily: '"Inter", system-ui, sans-serif', fontSize: 16, lineHeight: 2.15,
          ...
          {/* Safe space note */}
          <p style={{ margin: '0 0 20px', fontSize: 11, color: 'var(--ink-muted)', fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', opacity: 0.7 }}>
          ...
          <div style={{ marginTop: 4 }}>
            <input
              style={{
                fontFamily: '"Inter", system-ui, sans-serif', fontSize: 15, fontStyle: 'italic',
                color: 'var(--ink-muted)', background: 'transparent',
                border: 'none', borderBottom: '1px solid rgba(59,54,99,0.1)',
      ...
      {/* Crisis banner */}
      {showCrisis && (
        <div className="mt-4 rounded-xl px-5 py-4 animate-fade-up" style={{ background: 'rgba(46,125,91,0.08)', border: '1px solid rgba(46,125,91,0.22)' }}>
      ...
      <div className="mt-7 flex items-center gap-3 flex-wrap justify-end">
        ...
        {saveError && (
          <div style={{ fontSize: 12, padding: '9px 14px', borderRadius: 8, color: 'var(--tc)', background: 'rgba(244,129,63,0.07)', border: '1px solid rgba(244,129,63,0.18)', flexShrink: 0 }}>
            {saveError}
          </div>
        )}
        ...
        {isSelf ? (
          <button
            onClick={onSend}
            disabled={saving}
            style={{ padding: '9px 22px', borderRadius: 999, fontSize: 13, fontFamily: '"Inter", system-ui, sans-serif', fontWeight: 500, background: 'var(--tc)', color: 'white', border: 'none', cursor: 'pointer', transition: 'all 0.2s', opacity: saving ? 0.6 : 1 }}
            onMouseEnter={e => { if (!saving) { e.currentTarget.style.background = '#E06B28'; e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(244,129,63,0.3)' } }}
          ...
          <button
            onClick={onSendEmail}
            style={{ padding: '9px 22px', borderRadius: 999, fontSize: 13, fontFamily: '"Inter", system-ui, sans-serif', fontWeight: 500, background: 'var(--ink)', color: 'var(--cream)', border: 'none', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--tc)'; e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(244,129,63,0.25)' }}
    ...
    <div className="flex flex-col items-center justify-center text-center min-h-[60vh] py-12 animate-fade-up">
      <div className="mb-8">
        <svg width="90" height="68" viewBox="0 0 120 90" fill="none">
          <rect x="4" y="18" width="112" height="68" rx="4" fill="#FFFFFF" stroke="rgba(59,54,99,0.1)" strokeWidth="1"/>
          <path d="M4 22L60 56L116 22" fill="none" stroke="rgba(59,54,99,0.08)" strokeWidth="1"/>
          <path d="M4 86L40 58" stroke="rgba(59,54,99,0.07)" strokeWidth="0.8"/>
          <path d="M116 86L80 58" stroke="rgba(59,54,99,0.07)" strokeWidth="0.8"/>
          <circle cx="60" cy="56" r="12" fill="#8B3A2A"/>
          <circle cx="60" cy="56" r="10" fill="#F4813F"/>
      ...
      <h2 style={{ fontFamily: '"Inter", system-ui, sans-serif', fontSize: 'clamp(24px, 3.5vw, 34px)', fontWeight: 700, color: 'var(--ink)', lineHeight: 1.15, marginBottom: 12, letterSpacing: '-0.4px' }}>
        Your letter is<br /><em style={{ fontStyle: 'italic', color: headingColor }}>{headingText}</em>
      </h2>
      <p style={{ fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', fontSize: 15, color: 'var(--ink-soft)', maxWidth: 420, lineHeight: 1.75, margin: '0 auto 32px' }}>{msg}</p>
      <div className="flex gap-3 justify-center flex-wrap mb-8">
        <div className="flex items-center gap-2 rounded-pill text-[12px] text-ink-soft" style={{ padding: '9px 16px', border: '1px solid rgba(59,54,99,0.1)', background: 'var(--paper)' }}>
      ...
      <div className="flex gap-3 flex-wrap justify-center">
        <button
          ...
          style={{ padding: '12px 24px', borderRadius: 999, fontSize: 13, fontFamily: '"Inter", system-ui, sans-serif', color: 'var(--ink-soft)', background: 'transparent', border: '1px solid rgba(59,54,99,0.15)', cursor: 'pointer', transition: 'color 0.2s' }}
        ...
        <button
          ...
          style={{ padding: '12px 24px', borderRadius: 999, fontSize: 13, fontFamily: '"Inter", system-ui, sans-serif', fontWeight: 500, background: 'var(--ink)', color: 'var(--cream)', border: 'none', cursor: 'pointer', transition: 'all 0.2s' }}
        ...
        <div className="w-full max-w-3xl lg:max-w-4xl">
          ...
          <div>
            ...
            <aside className="hidden lg:flex flex-col gap-4" style={{ alignSelf: 'start', position: 'sticky', top: 68 }}>
              ...
              {/* Mood */}
              {mood && (
                <div style={{ background: 'var(--paper)', border: '1px solid rgba(59,54,99,0.08)', borderRadius: 12, padding: '14px 16px' }}>
                  ...
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 999, background: mood.activeStyle.background, color: '#fff', fontSize: 12, fontFamily: '"Inter", system-ui, sans-serif', fontWeight: 500 }}>
                    {mood.emoji} {mood.label}
                  </span>
                  <button
                    onClick={() => setStep(2)}
                    style={{ display: 'block', marginTop: 8, fontSize: 11, color: 'var(--tc)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: '"Inter", system-ui, sans-serif', padding: 0, textDecoration: 'underline', textUnderlineOffset: 2 }}
                  >
                    Change mood →
                  </button>
                </div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: deliveryType === opt.id && isKnown ? '1px solid var(--tc)' : '1px solid rgba(59,54,99,0.09)',
                        background: deliveryType === opt.id && isKnown ? 'rgba(244,129,63,0.06)' : '#fff',
                    ...
                    >
                      ...
                      {opt.comingSoon && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 10, background: 'rgba(59,54,99,0.06)', color: 'var(--ink-muted)' }}>Coming soon</span>}
                    </button>
                  ))}
                </div>
                {!isKnown && (
                  <p style={{ fontSize: 10.5, color: 'var(--ink-muted)', fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', marginTop: 8, lineHeight: 1.55 }}>
              ...
              {/* Privacy */}
              <div style={{ background: 'rgba(46,125,91,0.06)', border: '1px solid rgba(46,125,91,0.18)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                <p style={{ fontSize: 11.5, fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', color: 'var(--ink-soft)', lineHeight: 1.65, margin: 0 }}>
                  {mode === 'stranger'
                ...
                <div style={{ background: 'var(--paper)', border: '1px solid rgba(59,54,99,0.08)', borderRadius: 12, padding: '14px 16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--ink-muted)', fontFamily: '"Inter", system-ui, sans-serif', fontStyle: 'italic', marginTop: 1 }}>Platform default · always secure</div>
                    </div>
                    <span style={{ flexShrink: 0, fontSize: 9, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', padding: '3px 8px', borderRadius: 20, background: 'rgba(46,125,91,0.12)', color: 'var(--sage)', border: '1px solid rgba(46,125,91,0.25)', fontFamily: '"Inter", system-ui, sans-serif' }}>
                      Active
                    </span>
                  </div>
                </div>
