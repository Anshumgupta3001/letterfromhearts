...
export function HomeSidebar() {
  ...
  const stats = [
    { label: 'Personal',       value: personalLetters.length, icon: '🪞', page: 'myspace',       color: 'var(--purple)' },
    { label: 'To strangers',   value: strangerLetters.length, icon: '🌿', page: 'caringstranger', color: 'var(--sage)'   },
    { label: 'Sent',           value: sentLetters.length,     icon: '📤', page: 'sentletters',    color: 'var(--tc)'     },
    { label: 'Email accounts', value: emailAccounts.length,   icon: '📮', page: 'connections',    color: 'var(--gold)'   },
  ]
  ...
  return (
    ...
    >
      {/* Stats */}
      <div>
        <div className="text-[9.5px] tracking-[2px] uppercase font-medium text-ink-muted mb-3">Overview</div>
        <div className="flex flex-col gap-1.5">
          {stats.map(s => (
            <div
              key={s.label}
              onClick={() => navigate(s.page)}
              className="flex items-center justify-between px-[11px] py-[10px] rounded-[10px] cursor-pointer transition-all duration-150 hover:bg-warm"
              style={{ border: '0.5px solid transparent' }}
            >
              <div className="flex items-center gap-2.5 text-[13px]" style={{ color: 'var(--ink-soft)' }}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="text-[9.5px] tracking-[2px] uppercase font-medium text-ink-muted">Email</div>
          <span
            className="text-[10px] text-ink-muted cursor-pointer hover:text-ink transition-colors"
        ...
        {emailAccounts.length === 0 ? (
          ...
          >
            ...
            <div className="text-[11px] font-medium mt-1.5" style={{ color: 'var(--tc)' }}>+ Connect one →</div>
          </div>
        ) : (
          <div className="flex flex-col gap-1.5">
...
export function ListenSidebar({ filter, setFilter }) {
  ...
  return (
    <div className="border-r flex flex-col gap-6 px-[18px] py-7 sticky top-14 h-[calc(100vh-56px)] overflow-y-auto" style={{ borderColor: 'rgba(28,26,23,0.07)', background: 'rgba(247,242,234,0.4)' }}>
      ...
      <div>
        <div className="text-[9.5px] tracking-[2px] uppercase font-medium text-ink-muted mb-2.5">They need</div>
